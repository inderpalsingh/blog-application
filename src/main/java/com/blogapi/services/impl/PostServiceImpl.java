...
public class PostServiceImpl implements PostService {
    ...
    public PostServiceImpl(PostRepository postRepository, CategoryRepository categoryRepository, UserRepository userRepository, ModelMapper modelMapper, FileStorageService fileStorageService,ObjectMapper objectMapper) {
    ...
    /*
            When to use what?
            ObjectMapper → when dealing with JSON.
            ModelMapper → when converting one Java class to another, e.g., Entity → DTO.
        */

    @Override
    @Transactional
    public PostDto updatePost(Integer postId, String postDtoJson, MultipartFile imageFile) {
        try {

            // Parse JSON string to PostDto
            PostDto postDto = objectMapper.readValue(postDtoJson, PostDto.class);

            log.info("📥 Updating post {}: title={}, hasImage={}", postId, postDto.getTitle(), imageFile != null);
            ...
            log.error("❌ Failed to parse postDto JSON for post {}: {}", postId, e.getMessage());
            try {
                throw new BadRequestException("Invalid postDto JSON format: " + e.getMessage());
